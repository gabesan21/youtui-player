...
2. Run `makepkg -si`.
   ...
   - The `PKGBUILD` downloads `youtui-player-1.3.2.tar.gz` from the `gabesan21/youtui-player` release.
...
- **Builds the fork release, not the working tree.** The `PKGBUILD` `source=` downloads the `gabesan21/youtui-player` v1.3.2 tarball, so `makepkg -si` builds and installs the released fork, not the local working tree. Adapting the package to build the local tree is Epoch 2 scope.
