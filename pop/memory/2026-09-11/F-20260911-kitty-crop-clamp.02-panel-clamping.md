---
task: F-20260911-kitty-crop-clamp
entry: 02-panel-clamping
---

# Panel clamping for Kitty placements

tview clips cells when drawing, so the blocks path never leaked — but Kitty placements honor the raw `GetInnerRect` even for rows clipped by the panel border (latent bug surfaced by images). Every desired placement is now intersected with the owning canvas (list container / playerBox inner rect); fully clipped rects contribute no sink (Sync deletes stale placements), and partially visible rows map the clamped rect back into the source crop proportionally, mirroring tcell's clipping.

## Evidence

- [[internal/ui/kitty.go]] — *kittyRect.intersect and the partial-visibility math*.
- [[internal/ui/custom_list.go]] — *kittySinkSpecs clamped to the container rect*.
