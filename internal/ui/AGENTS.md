...
- Kitty sink keys are **content-keyed** (`prefix:videoID` parsed from the `i.ytimg.com/vi/<id>/` thumbnail URL, fallback the track URL), never `prefix:index` — an index shift (scroll, delete, move, page change) must not mis-pair a placement with another track. Image ids are **single-use**: every place emits a fresh transmit (new image id) plus a fresh placement id, and delete is `d=I` (frees placement AND image data) — reusing an image id is undefined when same-z placements overlap and breaks silently when a delete frees the data. A `renderVisibleItems` generation bump forces re-place at the next `Sync` even when a spec compares equal. `YOUTUI_KITTY_DEBUG=<path>` appends per-Sync decision logs; unset = no I/O.
