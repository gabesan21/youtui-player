...
- **i18n:** `internal/ui/i18n.go` provides PT-BR and EN string packs, auto-detected from `LC_ALL`/`LANG` or the config file; language changes apply in real time via `applyLanguage()`. `search.SetTexts()` is the cross-package bridge that keeps `internal/search` error messages in the active UI language.
