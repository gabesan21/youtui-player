...
**Playback config:** `[playback]` section in the TOML config exposes three user-facing options — `default_mode` (`"audio"`/`"video"`), `video_quality` (`"best"`, `"360"`, `"480"`, `"720"`, `"1080"`), and `video_codec` (`""`, `"vp9"`, `"av1"`). These are read at startup and editable in the settings view (Ctrl+C), a `tview.Form` built by `buildConfigForm()` in `setup.go` (dropdowns for language/theme/quality/codec, applied live; an input field for the download dir). The values are translated into a `--ytdl-format` selector by `buildYtdlFormat()` in `player.go` and passed to mpv at play time.
